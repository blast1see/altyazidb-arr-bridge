...
# What we want: a lightweight endpoint that
#   1. Accepts ?apikey= query auth (no cookie required)
#   2. Returns 200 and a small caps payload on valid key
#   3. Returns 401/403 on invalid key (so Test Jackett can distinguish)
$tests = @(
    "/api/v2.0/indexers/all/results/torznab/api?t=caps"
)
foreach ($e in $tests) {
...
foreach ($e in @("/api/v2.0/indexers/all/results/torznab/api?t=caps")) {
